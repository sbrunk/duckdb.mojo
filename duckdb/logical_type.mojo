...
struct LogicalType[is_owned: Bool, origin: ImmOrigin](ImplicitlyCopyable & Movable & Equatable & Writable):
    ...
    def __init__(out self, *, copy: Self):
        comptime if Self.is_owned:
            if copy.get_type_id() == DuckDBType.list:
                ...
                var size = copy.array_type_array_size()
                ref libduckdb = DuckDB().libduckdb()
                self._logical_type = libduckdb.duckdb_create_array_type(
                    child.internal_ptr(), size
                )
                ...
                for i in range(child_count):
                    c_names.append((base_ptr + i)[].as_c_string_slice().unsafe_ptr().as_unsafe_any_origin())
                ref libduckdb = DuckDB().libduckdb()
