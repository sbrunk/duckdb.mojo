...
struct Vector[lifetime: ImmutableLifetime]:
    ...
    fn __init__(inout self, vector: duckdb_vector, chunk: Reference[Chunk, lifetime], length: UInt64):
        self._vector = vector
        self._chunk = chunk
        self.length = length
    ...
    fn list_get_child(self) -> Vector[lifetime]:
        ...
        * returns: The child vector
        """
        return Vector(_impl().duckdb_list_vector_get_child(self._vector), self._chunk, _impl().duckdb_list_vector_get_size(self._vector))
    ...
    fn get_values[T: DBVal](self) raises -> List[Optional[T]]:
        ...
        if type == DuckDBType.timestamp_s:
            raise Error("Timestamps with second precision are not supported yet")
        if type == DuckDBType.timestamp_ms:
            raise Error("Timestamps with millisecond precision are not supported yet")
        if type == DuckDBType.timestamp_ns:
            raise Error("Timestamps with nanosecond precision are not supported yet")
    ...
    fn get_fixed_size_values[T: DBVal](self, length: Int, offset: Int) raises -> List[Optional[T]]:
        var data_ptr = self._get_data().bitcast[T]()
