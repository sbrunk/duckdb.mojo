...
def test_result_fetch_all():
    ...
    for i in range(10):
        assert_equal(result.get[Int64](col=0, row=i).value(), Int64(i))
...
def test_result_iteration():
    ...
    while iter.__has_next__():
        ...
        for i in range(len(chunk)):
            assert_equal(chunk.get[Int64](col=0, row=i).value(), Int64(count))
            count += 1
...
def test_result_with_null_values():
    ...
    assert_false(chunk.get[Int32](col=0, row=1))
...
def test_result_mixed_types():
    ...
    var chunk = result.fetch_chunk()
    assert_equal(chunk.get[Int32](col=0, row=0).value(), 42)
    assert_equal(chunk.get[Float64](col=1, row=0).value(), 3.14)
    assert_equal(chunk.get[String](col=2, row=0).value(), "hello")
    assert_equal(chunk.get[Bool](col=3, row=0).value(), True)
...
def test_result_large_integers():
    ...
    var chunk = result.fetch_chunk()
    assert_equal(chunk.get[Int64](col=0, row=0).value(), 9223372036854775807)
...
def test_result_negative_integers():
    ...
    var chunk = result.fetch_chunk()
    assert_equal(chunk.get[Int32](col=0, row=0).value(), -42)
    assert_equal(chunk.get[Int64](col=1, row=0).value(), -123456789)
