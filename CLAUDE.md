...
- FFI function-pointer types are emitted as `def(...) thin abi("C") -> ...` so `DLHandle.get_function` performs correct C-ABI struct coercion (the `abi("C")` effect was added in Mojo 1.0.0b1 and replaces the previous C shim workaround)
