...
**Extensions:** The `external_call` path is not available for extensions (shared libraries have no linker step against libduckdb). Extensions always use the DLHandle + C shim path and require `libduckdb_mojo_helpers` at runtime for affected functions.
