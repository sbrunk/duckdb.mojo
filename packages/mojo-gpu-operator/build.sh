...
case "$(uname -s)" in
Darwin) SOFLAGS=(-undefined dynamic_lookup) ;;
*) SOFLAGS=(-Wl,--allow-shlib-undefined) ;;
esac
...
echo "==> Mojo GPU kernels -> build/libmojo_gpu_kernels.dylib (shared-lib, runtime linked)"
mojo build --emit shared-lib "$HERE/src/gpu_kernels.mojo" -o "$BUILD/libmojo_gpu_kernels.dylib"
...
"$CXX" -std=c++17 -O2 -fPIC -shared "${SOFLAGS[@]}" \
	...
	-Wl,-rpath,@loader_path -Wl,-rpath,"$MOJO_LIB" \
